...
def run_roslaunch_and_check_rosbag(rospkg, launch_file, bag_file, x, y):
    ...
    # rosbag再生プロセスを取得する
    bag_process = subprocess.Popen(["rosbag", "play", "--clock", bag_file])
...
launch_file = "hdl_sim.launch"
bagfile = "/home/rokuto/08_01_1.bag"
...
noise_x, noise_y = np.array(rng.normal(0, 0.05, num_simulations)), np.array(rng.normal(0, 0.05, num_simulations))
for i in range(len(base_x_canditate)):
    for nx, ny in zip(noise_x, noise_y):
        ...
        new_x_canditate = float(base_x_canditate[i] + nx)
        new_y_canditate = float(base_y_canditate[i] + ny)
